...
## Station metadata — `data/lookups/stations.csv`

Sidecar metadata about each station (one row per `(source, site_id)`), joined to the observations above by `site_id`. Committed 40-station seed (expandable), regenerated by `scripts/build_stations_seed.py` from the CDSS climate-station catalog.

| Column | Type | Description |
| --- | --- | --- |
| `source` | string | `cdss_climate`. |
| `site_id` | string | CDSS station number — matches the output `site_id`. |
| `site_name` | string | Station name. |
| `network` | string | Federated source network (NOAA COOP/GHCN, CoCoRaHS, NRCS/SNOTEL, CoAgMet, NCWCD). |
| `site_id_network` | string | Native station id within that network. |
| `division` / `water_district` | string | CDSS administrative division / water district. |
| `county` | string | Colorado county. |
| `latitude` / `longitude` | float | Decimal degrees. |
| `start_date` / `end_date` | date | Station period of record. |
| `notes` | string | Provenance note. |

> No `elevation_ft` column yet — the committed seed predates it (the two coordinate-rich seeds carry elevation; adding it here is a tracked parity follow-up).
