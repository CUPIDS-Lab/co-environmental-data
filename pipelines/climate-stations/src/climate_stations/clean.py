...
Routes the source's saved responses through its parser, concatenates the canonical
frames, validates against the pandera schema, writes the deliverable
``data/processed/climate-stations.csv`` plus the ``provenance.csv`` sidecar.

Errors are **durable, not fatal**: a parser that raises on one artifact appends to
``data/audit/extraction_errors.json`` and the run continues, so one bad station
...
from climate_stations import config, provenance, schema
...
def run(*, sources: list[str] | None = None, sites: set[str] | None = None,
        meas_types: set[str] | None = None, fail_on_empty: bool = False,
    ...
    for slug, src in registry.items():
        if sources and slug not in sources:
            continue
        for art in src.discover(sites=sites, meas_types=meas_types):
            ...
            prov.append(provenance.ProvenanceRecord(
                ...
                parser_module=f"climate_stations.parsers.{slug}", source_license=src.license))
