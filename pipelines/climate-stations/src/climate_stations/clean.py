...
def run(*, sources: list[str] | None = None, sites: set[str] | None = None, meas_types: set[str] | None = None, fail_on_empty: bool = False,
    ...
    return _coreclean.run(
        config.get_sources(),
        discover=lambda src: src.discover(sites=sites, meas_types=meas_types),
        ...
        key_id_col="site_id", parser_module_prefix="climate_stations",
    )
