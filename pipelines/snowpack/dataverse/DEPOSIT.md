...
> **Keep the two-network caveat in the deposited metadata.** SNOTEL and snow courses are *nearby-not-identical* sites — complementary, not duplicate. This, plus the water-year-cumulative nature of `precip_accum_in` and the missing≠zero rule, is stated in `dataset.json`'s description and documented fully in `data/lookups/concepts.yaml` and the data dictionary.
...
> Run the pipeline first so `data/processed/*.csv` exists. The AWDB API needs **no key**: `uv run python -m snowpack.pipeline --mode live --fresh` (full state ~199 stations), or add `--sites …` for a sample.
...
A deposit run stops at a **draft** and offers to publish. **Review the draft in the web UI first** — publishing mints the DOI and is effectively permanent. Publish when ready by answering the prompt, or later with `./deposit-dataverse.sh --publish` (`--no-publish` to never publish). Before publishing, confirm the license (CC BY 4.0) and that the audit checks passed — in particular the **basin percent-of-normal reconciliation** vs. the NRCS basin report (`data/audit/reconcile-basin.json`) and the **cross-source co-location** check (`data/audit/reconcile-cross-source.json` — co-located pairs should track within a few inches).
