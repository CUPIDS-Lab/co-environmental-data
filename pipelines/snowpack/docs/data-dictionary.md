...
## Station metadata — `data/lookups/sites.csv`

Sidecar metadata about each station (one row per `(source, site_id)`), joined to the observations above by `site_id`. Full-state enumeration, regenerated by `scripts/build_sites_seed.py` from the NRCS AWDB station service.

| Column | Type | Description |
| --- | --- | --- |
| `source` | string | `nrcs_snotel` · `nrcs_snowcourse`. |
| `site_id` | string | AWDB station triplet (e.g. `303:CO:SNTL`) — matches the output `site_id`. |
| `station_id` | string | Numeric AWDB station id. |
| `site_name` | string | Station name. |
| `basin` | string | Colorado major-river basin (derived from `huc`). |
| `network` | string | `SNTL` (SNOTEL, automated daily) · `SNOW` (snow course, manual semimonthly). |
| `huc` | string | Hydrologic unit code. |
| `elevation_ft` | float | Station elevation, feet. |
| `latitude` / `longitude` | float | Decimal degrees. |
| `county` | string | Colorado county. |
| `begin_date` / `end_date` | date | Station period of record. |
