...
``data/processed/streamflow.csv`` plus the ``provenance.csv`` sidecar.

Errors are **durable, not fatal**: a parser that raises on one artifact appends
to ``data/audit/extraction_errors.json`` and the run continues, so one bad gage
...
from streamflow import config, provenance, schema
    ...
    for slug, src in registry.items():
        ...
        for art in src.discover(sites=sites):
            ...
            prov.append(provenance.ProvenanceRecord(
                ...
                parser_module=f"streamflow.parsers.{slug}", source_license=src.license))
