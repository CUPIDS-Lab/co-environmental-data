...
Sidecar metadata about each gage (one row per `(source, site_id)`), joined to the observations above by `site_id`. Hand-curated seed, regenerated by `scripts/build_sites_seed.py` from the USGS site service + the CDSS surface-water station catalog.

| Column | Type | Description |
| --- | --- | --- |
| `source` | string | `usgs_nwis` · `dwr_cdss`. |
| `site_id` | string | Source-native gage id (USGS site number / CDSS abbrev) — matches the output `site_id`. |
| `site_name` | string | Official gage name. |
| `basin` | string | Colorado major-river basin (curated). |
| `usgs_site_no` | string | USGS cross-link — the DWR mirror carries the same number (the join key). |
| `latitude` / `longitude` | float | Decimal degrees (USGS NAD83 ≈ WGS84). USGS from the site service; the DWR mirror from its own record — the two agree to ~5 decimals for a shared gage. |
| `elevation_ft` | float | Gage-datum elevation, feet (USGS `alt_va`, NAVD88/NGVD29). **DWR rows borrow the USGS value** — the surfacewaterstations record has no elevation, and it is the same physical gage. |
| `county` | string | Colorado county. |
| `start_date` / `end_date` | date | Daily-discharge period of record (USGS series catalog / DWR station POR). |
