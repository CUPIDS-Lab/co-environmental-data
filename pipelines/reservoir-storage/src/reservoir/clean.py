...
``data/processed/reservoir-storage.csv`` plus the ``provenance.csv`` sidecar.

Errors are **durable, not fatal**: a parser that raises on one artifact appends
to ``data/audit/extraction_errors.json`` and the run continues, so one bad
reservoir never empties the whole dataset. The notebook can pass
``fail_on_empty=True`` to turn a zero-row result (a silent layout regression)
into a hard error.
"""
...
from reservoir import config, provenance, schema
    ...
    for slug, src in registry.items():
        ...
        for art in src.discover():
            if not art.local_path.exists():
                continue  # not fetched yet — nb-01 retrieves first
            try:
            ...
            prov.append(provenance.ProvenanceRecord(
                ...
                parser_module=f"reservoir.parsers.{slug}", source_license=src.license))
