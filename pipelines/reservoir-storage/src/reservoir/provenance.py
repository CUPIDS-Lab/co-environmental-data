...
Provenance is per-*extract* (one row per ``(source, vintage)``), not per-row —
carrying the source URL on every observation is wasteful. The processed CSV
joins to this sidecar on ``(source, vintage)``. This is what makes the dataset
