...
| --- | --- | --- | --- |
| **Public** | The curated source catalog (`docs/planning/colorado_environmental_data_sources.json`); processed liberated datasets (e.g. the reservoir-storage CSV); all documentation; the planned Datasette + Quarto site, which shows **metadata + short excerpts + archived links only** — never republished article text. | Anyone. | No request needed; this is the default and the point. |
