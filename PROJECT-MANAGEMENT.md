...
Each task is one GitHub Issue, because an issue is the smallest unit a person can be assigned and held accountable for, and a single task per issue keeps that accountability legible. Issues for a given work batch are grouped under a **milestone** (here, **L1-L2: catalog hardening & pipeline**), so "what does this batch still owe" is one click away and closing the milestone marks it done. Issues then surface on the **Colorado Environmental Data Hub** Project board, which is where the team plans and tracks across views — board, table, and roadmap — rather than reading raw issue lists.

The current tracked batch is the *actionable-now* slice of the roadmap: harden the source catalog (verify the `needs_followup` sources, quarantine the spurious NREL `nlr.gov` claim, add the `match_hosts`/`match_keywords` citation-match fields) and stand up the L2 reproducible pipeline. The deferred L3–L5 concerns stay as roadmap text until the project climbs to them.
