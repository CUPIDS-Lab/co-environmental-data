...
- **Liberated datasets** (one present, more planned): tidy CSVs produced by `pipelines/<name>/` from public agency APIs — reservoir storage from CO DWR/CDSS + USBR Reclamation RISE is built (day-resolution, per-site, full history); streamflow and snowpack are planned. Unit of observation: one observation per site per day. Sources are reachable and re-releasable, so the pipeline regenerates results rather than depending on a frozen snapshot.
