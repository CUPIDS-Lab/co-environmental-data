...
- **Liberated datasets** (three present): tidy CSVs produced by `pipelines/<name>/` from public agency APIs — reservoir storage (CO DWR/CDSS + USBR Reclamation RISE), stream/river flow (USGS NWIS + CO DWR/CDSS), and snowpack/SWE (NRCS SNOTEL + snow courses via AWDB) are all built (day-resolution, per-site, full history). Unit of observation: one observation per site per day. Sources are reachable and re-releasable, so the pipeline regenerates results rather than depending on a frozen snapshot.
