...
| ◐ | Liberate CO water data — reservoir, streamflow, snowpack (*Water data liberation* milestone; reservoir #9 **built**) | _unassigned_ · help-wanted | High | L | series retrieved, tidied, documented, reconciled vs. source, republished to Datasette | No | L2 | [#7](https://github.com/CUPIDS-Lab/co-environmental-data/issues/7) |
...
> **Epic #7 breaks into per-domain sub-issues:** [#9](https://github.com/CUPIDS-Lab/co-environmental-data/issues/9) reservoir storage — **built** in [`pipelines/reservoir-storage/`](pipelines/reservoir-storage/) (CO DWR/CDSS + USBR Reclamation RISE live; full per-site history; tidy day-resolution CSV; provenance + concept caveats; Northern Water resolved as boundaries-only, its C-BT reservoirs sourced from RISE; monthly CI refresh + Dataverse deposit kit scaffolded). *Remaining:* publishing only — the first **human-reviewed** Dataverse deposit (RISE is now **20/20** and the reconcile spot-check passed; optional Datasette). · [#10](https://github.com/CUPIDS-Lab/co-environmental-data/issues/10) stream/river flow and [#11](https://github.com/CUPIDS-Lab/co-environmental-data/issues/11) snowpack (SWE) — **not started** (stamp out from the reservoir pipeline). Each is a retrieve → tidy → document → audit → publish slice, linked as native GitHub sub-issues of #7.
