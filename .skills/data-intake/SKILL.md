...
Never edit raw data in place. Treat each pipeline's raw cache (`data/original/`) and its committed seeds (`data/lookups/`) as immutable, and likewise the curated source catalog (`docs/planning/colorado_environmental_data_sources.json`). Read them, write derived data to the pipeline's `data/processed/`, and make sure any derived output can be regenerated from the source APIs. A new catalog vintage is a **new file**, not an in-place edit. Editing the source destroys your ability to audit against it and to reproduce your own work.
