...
This repo follows the CUPIDS Lab data-project conventions. **Code & data:** each **`pipelines/<name>/`** is a self-contained data-liberation pipeline (one per liberated dataset) that owns its **own** `data/` tree — `data/original` (immutable raw cache, never edited in place), `data/processed` (derived output, always regenerable from the source APIs), `data/lookups` (committed station seeds), and `data/audit` (profiles); **`pipelines/_core/`** is the shared [`co_pipeline_core`](pipelines/_core/README.md) library they all build on (provenance · fetch · clean · schema · audit, extracted in #54). There is **no top-level `data/` tree** — the repo's other shipped dataset, the curated **source catalog**, lives with the planning docs at [`docs/planning/colorado_environmental_data_sources.json`](docs/planning/colorado_environmental_data_sources.json); `context/` holds the project's design memos (methodology, architecture, source-inventory audit, proposals). **Documentation & governance:** the catalog schema is [`docs/reference/DATA-DICTIONARY.md`](docs/reference/DATA-DICTIONARY.md), and the planning, reference, governance, and responsible-data docs are organized under **[`docs/`](docs/)** — indexed in [Documentation](#documentation) below; the nested `.skills/` give downstream agents task-specific guidance (data intake, documentation, release). See [`docs/planning/ROADMAP.md`](docs/planning/ROADMAP.md) for what is planned but not yet built (the corpus pipeline and the L5 open-knowledge publication layer).
...
> **Publication is gated by QA.** Before any pipeline's data is deposited, its output must clear the bulletproofing + responsible-data checklists. The 2026-06-22 audit ([`audits/qa/2026-06-22-qa-audit.md`](audits/qa/2026-06-22-qa-audit.md)) found the reservoir output **not yet publish-ready** — the reconciliation is stubbed and the output carries impossible values — so the first Dataverse deposit (#36) is **blocked** on #38–#40.
...
python3 -m json.tool docs/planning/colorado_environmental_data_sources.json | less
```
...
Also: `CONTRIBUTING.md` (how to work here) · `CODE_OF_CONDUCT.md` · `CHANGELOG.md` (what changed and when) · [`context/`](context/) (methodology memo, technical architecture, source-inventory audit) · `.skills/` (nested agent guidance) · [`audits/`](audits/) — point-in-time QA audits; see [`audits/qa/2026-06-22-qa-audit.md`](audits/qa/2026-06-22-qa-audit.md) (reservoir — what blocks the first publish) and [`audits/qa/2026-06-23-pipelines-qa.md`](audits/qa/2026-06-23-pipelines-qa.md) (streamflow / snowpack / climate-stations on the shared core).
